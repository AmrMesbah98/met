...
class SessionProvider extends ChangeNotifier {
  ...
  Future<List<Session>> getLectures({
    required String lecturerId,
  }) async {
    ...
    return lecturerDoc.docs
        .map((element) => Session.fromJson(element.data()))
        .toList();
  }

  Future<Session?> getCurrentLecture({required String lecturerId}) async {
...
//create subject
  ...
  Future<void> saveData({
    ...
    final sessionDoc =
        FirebaseFirestore.instance.collection('sessions').doc(session.id);
    var updatedSessions = session;
    if (session.attendedStudentIds == null) {
      updatedSessions = updatedSessions.copyWith(attendedStudentIds: []);
    }
    updatedSessions = session.copyWith(
      attendedStudentIds: [
        ...session.attendedStudentIds!,
        studentId,
      ],
    );
    await sessionDoc.update(updatedSessions.toJson());
  }

  Future<List<UserModel>?> getSessionsAttendance({
    ...
    return students.docs.map((e) => UserModel.fromJson(e.data())).toList();
  }
}
