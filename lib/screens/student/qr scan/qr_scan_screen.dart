...
import 'package:metqr/screens/student/student%20home%20page/stu_homepage_screen.dart';
...
class _QrScanPageState extends State<QrScanPage> {
  ...
  void onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) {
      print(scanData.code);
      if (scanData.code!.contains('lecturerId')) {
        final parseData = json.decode(scanData.code!);
        if (parseData != null) {
          final session = Session.fromJson(parseData);
          Provider.of<SessionProvider>(context, listen: false).saveData(
            session: session,
            studentId: Provider.of<StudentAuthProvider>(context, listen: false)
                .studentModel!
                .id!,
          );
          controller.pauseCamera();
          // navigateTo(context, const StudentHomePage());
        }
      }
      setState(() {
        result = scanData;
      });
      debugPrint('xxxxxxxxx => ${scanData.code}');
    });
  }
}
